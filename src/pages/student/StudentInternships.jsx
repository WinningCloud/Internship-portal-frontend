...
const StudentInternships = () => {
  ...
  useEffect(() => {
    applyFilters();
  }, [search, domain, sortBy, internships]);
  ...
  const applyFilters = () => {
    ...
    // 🏷 Domain filter
    if (domain !== "ALL") {
      temp = temp.filter((i) => i.domain === domain);
    }
...
const InternshipCard = ({ data, navigate }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={`p-6 rounded-[2.5rem] border ${color} flex flex-col h-full justify-between transition-all`}
    >
      <div>
        ...
        <h3 className="text-xl font-black text-slate-900 mb-4 leading-tight line-clamp-3">
          {data.title}
        </h3>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 bg-white border border-slate-200 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all"
        >
          View Details
        </button>
        <button className="p-3.5 bg-slate-900 text-white rounded-2xl">
          <ArrowRight size={18} />
        </button>
      </div>
    </motion.div>
